...
impl Indicator {
    pub fn new(config: config::Indicator, text_height: f64) -> Self {
        ...
        Self {
            ...
            background: Pattern::get_pattern(diameter - border_width, config.background, config.background_stop),
            border_color: config.border_color.into(),
            border_color_focused: config.border_color_focused.into(),
            indicator_color: Pattern::get_pattern(diameter - border_width, config.indicator_color, config.indicator_color_stop),
            has_focus: false,
            pass_len: 0,
            dirty: false,
            dirty_blink: false,
...
impl<'a> XcbSurface<'a> {
    pub fn new(
        ...
        let pixmap = conn
            .generate_id()
            .map_err(|e| conn.xerr_from("pixmap generate id", e))?;
    ...
    pub fn setup_pixmap(
        ...
        let pixmap = self
            .conn
            .generate_id()
            .map_err(|e| self.conn.xerr_from("pixmap generate id", e))?;
        self.conn
